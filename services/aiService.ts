...
const AI_CONFIG = {
    simulationDelay: 500,
    modelName: 'Agricultural-Intelligence-v1',
    timeout: 10000
};
...
export const analyzeFincaData = async (data: AppState): Promise<string> => {
    // 1. Network Check
    if (!navigator.onLine) {
        return "⚠️ Sin conexión a Internet. El análisis de IA requiere conexión para procesar los modelos avanzados. Por favor, verifique su red.";
    }

    try {
        // Simular latencia de red/proceso
        await new Promise(resolve => setTimeout(resolve, AI_CONFIG.simulationDelay));
